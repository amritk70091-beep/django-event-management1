...
from .forms import EventForm, EventImageForm, EventAgendaForm


# Event category list view
# Add these to the bottom of views.py
...
class AddEventMemberCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.created_user = self.request.user
        form.instance.updated_user = self.request.user
        return super(AddEventMemberCreateView, self).form_valid(form)
...
class AddEventUserWishListCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.created_user = self.request.user
        form.instance.updated_user = self.request.user
        return super(AddEventUserWishListCreateView, self).form_valid(form)
...
class CreateUserMark(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.created_user = self.request.user
        form.instance.updated_user = self.request.user
        return super(CreateUserMark, self).form_valid(form)
